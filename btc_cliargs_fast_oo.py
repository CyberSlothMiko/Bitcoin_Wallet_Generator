...
def generate_output(filetype):
    ...
    match filetype:
        ...
        case "priv_nl":
            generate_priv_or_pub_csv_nl("priv_nl.txt",WIF_Key_Output,"",filetype)
        case "pub_nl":
            generate_priv_or_pub_csv_nl("pub_nl.txt",Public_Key_Output,"",filetype)
        case "priv_csv":
            generate_priv_or_pub_csv_nl("priv_csv.csv",WIF_Key_Output,",",filetype)
        case "pub_csv":
            generate_priv_or_pub_csv_nl("pub_csv.csv",Public_Key_Output,",",filetype)
        case "priv_pub_csv_seperate":
...
def generate_fancy_txt(filename):
        ...
        if wallet_counter > 1:
            with open(filename, 'a') as output_file:
                while output_counter < wallet_counter:
                    ...
                    output_counter += 1
    elif last_line != "==================================================================================" and wallet_counter > 1:
        with open(filename, 'a') as output_file:
                while output_counter < wallet_counter:
                    output_file.write("==================================================================================\n")
                    output_file.write("Bitcoin Private Key (WIF)  : " + Output_key_list[0][output_counter]  + "\n")
    ...
    if output_counter == wallet_counter:
        with open(filename, 'a') as output_file:
            output_file.write("==================================================================================")
    else:
        with open(filename, 'a') as output_file:
...
def generate_priv_or_pub_csv_nl(filename,data,delimiter,filetype):
